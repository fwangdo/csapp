...
- 하나의 "물리적 프로세스(하나의 프로그램)"라고 하더라도, 여러개의 "논리적 프로세스"로 나누어 생각할 수 있다.
  - 예를 들어 공유 라이브러리 파일이 있는 경우, 실행하다가 메모리를 크게 점프해서 실행해야하는 경우가 있는데, 이런 경우를 논리적 프로세스가 나눠지는 구간으로 볼 수 있음.
