...
- G3. 공유되는 전역 데이터 구조(shared global data structure)에 대해 blocking으로 보호하기: 핸들러가 메인 프로그램과 함께 공유되는 데이터(d)에 대한 권한을 가지고 있다고 하자. 핸들러를 통해 해당 데이터가 만져질 경우, 프로그램 입장에서는 d를 신뢰하기가 어렵다. 변했을 수 있으니까.
