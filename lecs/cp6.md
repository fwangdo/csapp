...
- 정의를 찾지 못하는 경우: 참조되었으나 정의되지 않은 변수 / 함수를 만날 경우, os까지 뒤져가면서 찾음. 이때 에러메세지는 알기 어려움.
- 정의가 여러개인 경우: 여러개 중 하나를 선택하고 다른 것들은 버리르 방향으로. 이때 리눅스 시스템이 쓰는 방법은 다음과 같다.
  - 먼저 strong / weak symbol을 구분한다. strong에는 함수 / 초기화된 전역변수가 포함되며, weak는 지역변수, 초기화되지 않은 전역변수가 포함된다.
  - 1. 여러개의 "strong variable"이 존재한다면, 컴파일이 거부된다.
  - 1. 하나의 strong, 여러개의 weak이 있다면 strong을 선택한다.
  - 1. 여러개의 weak만 있다면 아무거나 선택한다.
  - p.708의 foo3 케이스를 참조하면 좋음.

### static library

- 여러 개의 relocated file을 하나로 묶어주는 역할. 컴파일러는 이걸 복사해서 실행파일을 만듦.
- 왜 필요한가? 아래 방식을 고민해보면 정적 라이브러리가 좋은 이유가 보인다.
  - 컴파일러에 공유 라이브러리 구현을 직접 적어두자: 이건 말도 안 되고.
  - .o 파일에 몰아넣자: 이러면 프로그램이 항상 .o 파일을 모두 복사해야해서 크기가 너무 커짐.
  - 자잘하게 부숴서 하나의 디렉토리 안에 넣어두면?: 링킹하기가 너무 힘듦(링킹 파일 모두 나열!)
- 이런 이유로 심볼 테이블이 존재해서 하나의 파일만 존재하되(.a), 컴파일시 필요한 것만 복사할 수 있는 정적 라이브러리가 좋음.
