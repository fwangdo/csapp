...
- loop unrolling을 과도하게 하면(e.g., 20 by 20) 오히려 더 성능이 안 좋아질 수도.
  - 레지스터가 담아있는 범위를 초과해서 참조해야할 값들을 메모리로 위치시킬 수도 있기 때문. 이걸 register spilling이라고 칭함.
  - 인텔의 x86의 레지스터가 16개로 구성되어 있다는 것을 감안할 것.

### conditional move에 대한 이해

- 일반적으로 branch prediction을 피하기위해 양쪽을 모두 계산하고 계산할 때 "조건적으로 값을 반환하는 형태"의 cmov 계열을 사용
  - Q. 그래도 결국 "최종값이 결정되어야" 이후 계산을 수행할 수 있는 것 아닌가?
